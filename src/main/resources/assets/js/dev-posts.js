$(function () {
    ...
    const API_DEV_POST_PAGINATION_URL = '/a/dev-posts/pagination';
    const POST_TYPE = 'POST';
    const $DEV_POST_LIST = $('#grid');
    const DEV_POST_LIST_CATEGORY = getParameterByName('c');
    const DEV_POST_LIST_TAG = getParameterByName('t');
    ...
    const DEV_POST_TEMPLATE = '<article class="box-item"><span class="category"><a href="/category?c={{category}}"><span>{{category}} - {{postType}}</span></a></span><div class="box-body">' +
            '<a class="cover view-anchor" href="{{url}}" target="_blank" data-id="{{id}}"><svg width="50" height="50" viewBox="0 0 38 38" xmlns="http://www.w3.org/2000/svg" class="loader"><defs>' +
        ...
        '</a><div class="box-info"><time datetime="{{uploadAt}}" class="date">{{uploadAt}}</time><a class="post-link view-anchor" href="{{url}}" target="_blank" data-id="{{id}}"><h2 class="post-title">{{title}}</h2></a>\<a class="post-link view-anchor" href="{{url}}" target="_blank">' +
        '<p class="description">{{description}}</p></a>{{tags}}</div></div></article>';
    ...
    devPostDrawling(DEV_POST_LIST_CATEGORY, DEV_POST_LIST_TAG, getParameterByName('page'));
    paginationDrawling(DEV_POST_LIST_CATEGORY, DEV_POST_LIST_TAG, getParameterByName('page'));

    function getAllSiteParameter() {
        if (DEV_POST_LIST_CATEGORY !== '' && DEV_POST_LIST_TAG !== '') {
            if (DEV_POST_LIST_CATEGORY === 'c') {
                return '?c=' + DEV_POST_LIST_TAG + '&page=';
            } else if (DEV_POST_LIST_TAG === 't') {
                return '?t=' + DEV_POST_LIST_TAG + '&page=';
            }
        } else {
            return '?page=';
        }
    ...
    $(document).on('click', '.view-anchor', function () {
        $.ajax({
            url: API_DEV_POST_URL + '/' + $(this).data('id'),
            type: 'PATCH',
        ...
        $('html, body').stop().animate({
            scrollTop: $DEV_POST_LIST.offset().top - $('.bar-header').eq(0).innerHeight()
				...
				if ($(this).hasClass('page-arrow')) {
					paginationDrawling(DEV_POST_LIST_CATEGORY, DEV_POST_LIST_TAG, VALUE);
				} else {
					$('.page-item').removeClass('page-active');
          $(this).addClass('page-active');
				}

        devPostDrawling(DEV_POST_LIST_CATEGORY, DEV_POST_LIST_TAG, VALUE);
    });
    ...
    function devPostDrawling(c, t, page) {
        ...
        if (c !== undefined && c !== '') formData.c = c;
        if (t !== undefined && t !== '') formData.t = t;
        if (page !== undefined && page !== '') formData.page = page;
                ...
                if (data.success) {
                  ...
                  for (const post of data.response) {
                      let template = DEV_POST_TEMPLATE.replaceAll(MATCH_TITLE, post.title)
                          .replaceAll(MATCH_UPLOAD_AT, new Intl.DateTimeFormat('en-US', {year: 'numeric', month: 'short', day: 'numeric'}).format(new Date(post.uploadAt)))
                          .replaceAll(MATCH_ID, post.id)
                      ...
                      template = template.replaceAll(MATCH_TAGS, tagTemplate);
    ...
    function paginationDrawling(c, t, page) {
      ...
      $.ajax({
        url: API_DEV_POST_PAGINATION_URL,
        type: 'GET',
        data: formData,
        success: function (data) {
           if (data.success) {
             ...
             if (data.response.previousPageNum) {
